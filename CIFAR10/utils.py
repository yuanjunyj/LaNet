...
def mask2config_nasbench301(mask):
    OPS = [
        'max_pool_3x3',
        'avg_pool_3x3',
        'skip_connect',
    ...
    for cell_type in ['normal', 'reduce']:
        for i in range(2, 6):
            end = start + one_hot_len * i
            encoding = mask[start:end]
            for j in range(i):
                for k in range(one_hot_len):
                    if encoding[j * one_hot_len + k] > 0:
                        edges[cell_type].append((OPS[k], j))
            start = end
...
def mask2config_nasnet(mask):
    ...
    mask = np.array(mask).reshape(-1, 4)
    normal = []
    reduce = []
    normal_concat = []
    reduce_concat = []
    for i in range(2, node + 2):
